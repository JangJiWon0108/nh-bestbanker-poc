...
code_execution_agent = LlmAgent(
    ...
    description="수식 모델링 또는 full text 문서를 바탕으로 Python 코드를 실행해 계산 결과를 산출하는 에이전트",
...
- formula_modeling.modeling_status (없을 수 있음)
- formula_modeling.formula_expression (없을 수 있음)
- formula_modeling.variable_definitions (없을 수 있음)
- formula_modeling.assumptions (없을 수 있음)
- retrieval_context.retrieval_results (있을 수 있음, needs_calculation인 경우 full text 원문 목록)
- retrieval_context.search_scope
...
2) calculation_requirement가 no_calculation 이거나 formula_modeling.modeling_status가 not_required이면
   execution_status는 skipped_not_required로 반환한다.
3) formula_modeling.modeling_status가 modeled인 경우에는 해당 수식/변수 정의를 기준으로 코드를 작성/실행한다.
4) formula_modeling 단계가 없고, retrieval_context.search_scope가 "full_text_v2_local"인 경우에는
...
5) 실행 코드는 변수 선언 -> 계산 -> 최종 결과를 float 출력 순서로 작성한다.
   코드 문자열을 작성할 때 역슬래시 이스케이프("\\n")를 넣지 말고 실제 줄바꿈을 사용한다.
6) 코드 실행 결과에 Traceback, SyntaxError, OUTCOME_FAILED 등 실패 신호가 있으면
   execution_status=failed로 반환하고 result_value는 null로 둔다.
7) 실행 성공 시에만 execution_status=executed, result_value에 숫자 결과를 채운다.
8) 정보 부족/오류 시 execution_status=failed, result_text에 원인을 남긴다.
   formula_modeling.assumptions 또는 문서에서 확인한 부족 항목이 있으면 result_text에 핵심 부족 항목을 요약해 포함한다.
9) 코드 미실행인 경우 executed_code는 빈 문자열로 둔다.
10) executed_code에는 "실제로 실행한 Python 코드"만 넣는다.
    Outcome/Output/Traceback 같은 실행 로그 문자열은 executed_code에 포함하지 않는다.
11) 최종 응답은 아래 키를 가진 JSON 객체 1개만 반환한다.
    - execution_status
    - result_value
    - result_text
    - executed_code
