...
def retrieve_docs_by_categories(
    ...
    invalid_categories = [category for category in normalized_categories if category not in ALLOWED_CATEGORIES]

    # LLM이 잘못 생성한 카테고리는 예외를 던지지 말고 조용히 무시한다.
    # 허용된 카테고리만 남기고, 모두 사라지면 검색을 생략한다.
    normalized_categories = [
    ...
    if not normalized_categories:
        return {
            ...
            "total_size": 0,
            "message": (
                "검색 가능한 카테고리가 없어 검색을 생략했습니다."
                + (f" (invalid_categories={invalid_categories})" if invalid_categories else "")
            ),
        }
